...
export async function specSetup(autoRun = false) {
  ...
  if (packageManager === 'pnpm') {
    try {
      if (pathExists('pnpm-workspace.yaml')) {
        ...
        await createFileFromWebTemplate('https://raw.githubusercontent.com/AloisSeckar/nuxt-spec/refs/tags/v0.2.3/config/templates/pnpm-workspace.yaml.template',
  ...
  if (!pathExists('.nuxtrc')) {
    try {
      await createFileFromWebTemplate('https://raw.githubusercontent.com/AloisSeckar/nuxt-spec/refs/tags/v0.2.3/.nuxtrc', '.nuxtrc', isAutoRun, 'This will create a \'.nuxtrc\' file to prevent @nuxt/test-utils setup from running automatically when dev server starts. Continue?')
    } catch (error) {
