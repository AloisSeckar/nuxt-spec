...
If you don't want to use the CLI tool, or you want to understand its flow better, here are the detailed explanation of the required/recommended steps:
...
It is also advised to remove all explicit `nuxt`, `vue` and `vue-router` dependencies, if present. The `nuxt-spec` layer brings all of them and there might be version clashes if defined in both places.
...
The file will be auto-generated by Nuxt anyway, if you don't do it.
...
The struture matches the default [configuration](/2-1-configuration) of `nuxt-spec`.
...
**2)** If you're prompted (for the first time when installing to a new machine or after version update), locally install headless browser runtimes:
...
This is required for all auto-generated files gets emitted and all the features and type inference work properly.
